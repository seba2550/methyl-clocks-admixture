...
bio_age <- read.csv("data/bio_age_estimates_magenta_age_diff_metadata.csv", row.names = 1)
...
bio_age_global_anc_melted %>%
  filter(COHORT != "NHW") %>%
  ggplot(aes(x = ETHNICITY, y = proportion, fill = global_anc)) +
...
bio_age_global_anc_melted %>%
  filter(COHORT != "NHW") %>%
  ggplot(aes(x = COHORT, y = proportion, fill = global_anc)) +
  geom_boxplot() +
  theme_minimal()
...
p2_ggpubr_symmetric <- bio_age_global_anc %>%
  filter(COHORT == "REAAADI" & STATUS == "CONTROL") %>%
  ggscatter(
    x = "Horvath",
    y = "age",
  ...
  geom_abline(intercept = 0, slope = 1, color = "grey", linetype = "dashed") +
  coord_fixed() +
  expand_limits(x = c(60, 90), y = c(60, 90)) + # This replaces xlim/ylim
  theme(
    axis.title = element_text(size = 16),
    axis.text = element_text(size = 14),
    plot.title = element_text(size = 18)
...
p4_ggpubr_symmetric <- bio_age_global_anc %>%
  filter(COHORT == "PRADI" & STATUS == "CONTROL") %>%
  ggscatter(
    x = "Horvath",
    y = "age",
...
p6_ggpubr_symmetric <- bio_age_global_anc %>%
  filter(COHORT == "CuADI" & STATUS == "CONTROL") %>%
  ggscatter(
    x = "Horvath",
    y = "age",
...
p8_ggpubr_symmetric <- bio_age_global_anc %>%
  filter(COHORT == "PERUVIAN" & STATUS == "CONTROL") %>%
  ggscatter(
    x = "Horvath",
    y = "age",
...
fig2b_symmetric <- plot_grid(p2_ggpubr_symmetric, p1_ggpubr, p4_ggpubr_symmetric, p3_ggpubr,
  p6_ggpubr_symmetric, p5_ggpubr, p8_ggpubr_symmetric, p7_ggpubr,
  ncol = 4, align = "hv"
)

# Example assuming you already have them as individual ggplots
row1 <- plot_grid(p2_ggpubr_symmetric, p1_ggpubr, p4_ggpubr_symmetric, p3_ggpubr, ncol = 4, rel_widths = c(2, 1, 2, 1))
row2 <- plot_grid(p6_ggpubr_symmetric, p5_ggpubr, p8_ggpubr_symmetric, p7_ggpubr, ncol = 4, rel_widths = c(2, 1, 2, 1))

# Combine rows into panel B
fig2b <- plot_grid(row1, row2, ncol = 1, rel_heights = c(1, 1))

######### MAE distributions
# Filter, relabel, order, and compute abs values
# Prepare data
# df_plot <- bio_age_global_anc %>%
#   filter(STATUS == "CONTROL") %>%
#   mutate(
#     abs_ageAcc = abs(ageAcc.Horvath),
...
#   )
#
# # Compute cohort medians (explicitly)
# global_range <- range(df_plot$abs_ageAcc, na.rm = TRUE)
# offset <- 0.05 * (global_range[2] - global_range[1])  # small upward offset for labels
#
# medians <- df_plot %>%
#   group_by(COHORT) %>%
#   summarise(median_abs = mean(abs_ageAcc, na.rm = TRUE)) %>%
#   mutate(y_pos = median_abs + offset)
#
# # Optional: inspect medians to confirm values
# print(medians)
#
# # Color palette (your reference)
# cohort_colors <- c(
#   "Whites"              = "gray60",
#   "African Americans"   = "#00BFFF",
#   "Puerto Ricans"       = "#FF69B4",
#   "Cubans"              = "#9ACD32",
#   "Peruvians"           = "#20B2AA"
# )
#
...
#   trim = FALSE
# ) +
#   # add text labels using the precomputed medians (one per cohort)
#   geom_text(
#     data = medians,
#     aes(x = COHORT, y = y_pos, label = paste0("MAE = ", round(median_abs, 2))),
#     inherit.aes = FALSE,
#     vjust = 0,
#     size = 4
#   ) +
#   scale_fill_manual(values = cohort_colors) +
#   labs(x = "Cohort", y = "Absolute Age Acceleration (Horvath)") +
#   theme_pubr() +
#   rremove("legend")
#
# print(p)
...
#   )
#
# # Compute cohort means explicitly
# global_range <- range(df_plot$abs_ageAcc, na.rm = TRUE)
# offset <- 0.05 * (global_range[2] - global_range[1])  # space for labels
#
# means <- df_plot %>%
#   group_by(COHORT) %>%
#   summarise(mean_abs = mean(abs_ageAcc, na.rm = TRUE)) %>%
#   mutate(y_pos = mean_abs + offset)
#
# # Custom colors
# cohort_colors <- c(
#   "Whites"              = "gray60",
...
#   add = "jitter"   # adds a mean point to each box
# ) +
#   labs(
#     x = "Cohort",
...
spearman_correlation_df <- bio_age_global_anc %>%
  ...
  summarize(
    Horvath = cor(Horvath, age, method = "spearman"),
...
df <- df %>% mutate(
  ...
  pvalue = case_when(
    COHORT == "AA" & DNAmAge == "Horvath" ~ nhw_aa_compare[[1]]@fisher1925$p.value,
    COHORT == "AA" & DNAmAge == "Hannum" ~ nhw_aa_compare[[2]]@fisher1925$p.value,
    COHORT == "AA" & DNAmAge == "PhenoAge" ~ nhw_aa_compare[[3]]@fisher1925$p.value,
    COHORT == "AA" & DNAmAge == "Zhang2019_EN" ~ nhw_aa_compare[[5]]@fisher1925$p.value,
    COHORT == "PUR" & DNAmAge == "Horvath" ~ nhw_pr_compare[[1]]@fisher1925$p.value,
    COHORT == "PUR" & DNAmAge == "Hannum" ~ nhw_pr_compare[[2]]@fisher1925$p.value,
    COHORT == "PUR" & DNAmAge == "PhenoAge" ~ nhw_pr_compare[[3]]@fisher1925$p.value,
    COHORT == "PUR" & DNAmAge == "Zhang2019_EN" ~ nhw_pr_compare[[5]]@fisher1925$p.value,
    COHORT == "Cuban" & DNAmAge == "Horvath" ~ nhw_cub_compare[[1]]@fisher1925$p.value,
    COHORT == "Cuban" & DNAmAge == "Hannum" ~ nhw_cub_compare[[2]]@fisher1925$p.value,
    COHORT == "Cuban" & DNAmAge == "PhenoAge" ~ nhw_cub_compare[[3]]@fisher1925$p.value,
    COHORT == "Cuban" & DNAmAge == "Zhang2019_EN" ~ nhw_cub_compare[[5]]@fisher1925$p.value,
    COHORT == "Peruvian" & DNAmAge == "Horvath" ~ nhw_per_compare[[1]]@fisher1925$p.value,
    COHORT == "Peruvian" & DNAmAge == "Hannum" ~ nhw_per_compare[[2]]@fisher1925$p.value,
    COHORT == "Peruvian" & DNAmAge == "PhenoAge" ~ nhw_per_compare[[3]]@fisher1925$p.value,
    COHORT == "Peruvian" & DNAmAge == "Zhang2019_EN" ~ nhw_per_compare[[5]]@fisher1925$p.value
  ),
  human_readable_cohort_name = case_when(
    COHORT == "NHW" ~ "Whites",
    COHORT == "AA" ~ "African Americans",
    COHORT == "PUR" ~ "Puerto Ricans",
    COHORT == "Cuban" ~ "Cubans",
    COHORT == "Peruvian" ~ "Peruvians"
  )
)

df <- df %>% mutate(comp_pval = 1 - pvalue)
...
df <- df %>% mutate(human_readable_cohort_name = factor(human_readable_cohort_name, levels = c("Whites", "Cubans", "Peruvians", "African Americans", "Puerto Ricans")))

fig2c <- ggbarplot(
  df,
  x = "human_readable_cohort_name",
  y = "relative_correlation",
  fill = "human_readable_cohort_name",
  position = position_dodge(), # Same as position = "dodge"
  ylab = "Pearson Correlation (Relative to NHW)",
  xlab = "Cohort",
  ylim = c(-0.3, 0.3), # Set y-axis limits
  ...
  geom_text(
    aes(
      label = ifelse(COHORT == "NHW", round(NHW_correlation, 2), ""),
      y = relative_correlation + 0.05
    ),
    vjust = -0.5, size = 5, color = "black",
  ...
  geom_text(
    aes(
      label = ifelse(comp_pval < 0.05, "*", ""),
      y = 0.05 # Position asterisks just above the 0 line
    ),
...
df_horvath <- df %>%
  filter(DNAmAge == "Horvath") %>%
  mutate(
    human_readable_cohort_name = factor(
      human_readable_cohort_name,
      levels = c("Whites", "Cubans", "Peruvians", "African Americans", "Puerto Ricans")
    ),
    # Add manual confidence intervals for each cohort
    lower_ci = case_when(
      human_readable_cohort_name == "Whites" ~ NA_real_,
      human_readable_cohort_name == "African Americans" ~ -0.3949,
      human_readable_cohort_name == "Puerto Ricans" ~ -0.4964,
      human_readable_cohort_name == "Cubans" ~ -0.3834,
      human_readable_cohort_name == "Peruvians" ~ -0.2150
    ),
    upper_ci = case_when(
      human_readable_cohort_name == "Whites" ~ NA_real_,
      human_readable_cohort_name == "African Americans" ~ -0.0184,
      human_readable_cohort_name == "Puerto Ricans" ~ -0.0518,
      human_readable_cohort_name == "Cubans" ~ 0.1891,
      human_readable_cohort_name == "Peruvians" ~ 0.1871
    )
  )
...
p <- ggbarplot(
  ...
  ylab = "Pearson Correlation (Relative to Whites)",
  xlab = "Cohort",
  ylim = c(-0.6, 0.3),
  lab.pos = "out",
  lab.size = 5,
  lab.hjust = 0.5,
  lab.vjust = -0.5,
  title = "Horvath Clock Relative Accuracy"
) +
  geom_errorbar(
...
fig2c <- ggplot(df, aes(x = human_readable_cohort_name, y = relative_correlation, fill = human_readable_cohort_name)) +
  ...
  geom_text(
    aes(
      label = ifelse(COHORT == "NHW", round(NHW_correlation, 2), ""),
      y = ifelse(relative_correlation >= 0,
        relative_correlation + upper_ci + 0.03,
        relative_correlation - lower_ci - 0.03
      )
    ),
    vjust = ifelse(df$relative_correlation >= 0, -0.5, 1.5),
    size = 4,
    color = "black",
    position = position_dodge(width = 0.9)
  ) +
  geom_text(
    aes(
      label = ifelse(comp_pval < 0.05, "*", ""),
      y = ifelse(relative_correlation >= 0,
        relative_correlation + upper_ci + 0.10,
        relative_correlation - lower_ci - 0.10
      )
    ),
    vjust = ifelse(df$relative_correlation >= 0, -0.5, 1.5),
    size = 6,
  ...
  facet_wrap(~DNAmAge, scales = "free_x", nrow = 2) +
  scale_y_continuous(
    limits = c(-0.5, 0.5),
    breaks = seq(-0.50, 0.50, 0.25)
  ) +
  labs(
    y = "Pearson Correlation (Relative to Whites)",
    x = "Cohort"
  ) +
  theme_classic() +
  theme(
...
# fig2a_fixed <- plot_grid(fig2a, NULL, ncol = 1, rel_heights = c(1, 1))
# plot_grid(fig2a_fixed, fig2b, labels = c("A", "B"),
#           rel_widths = c(1,3),
#           ncol = 2)

fig2a_centered <- plot_grid(
  NULL, fig2a, NULL,
  ncol = 1,
  rel_heights = c(1, 1, 1) # middle gets more weight
)

fig2b <- plot_grid(row1, row2, ncol = 1, rel_heights = c(1, 1))

# Whites (Panel A), centered vertically across both rows
fig2a_spanned <- plot_grid(
  NULL, fig2a, NULL,
  ncol = 1,
  rel_heights = c(1, 2, 1)
)

# Combine: give A the same width "weight" as a scatterplot (2), and B the rest (6)
# final_plot <- plot_grid(
#   fig2a_spanned, fig2b,
#   labels = c("A", "B"),
#   ncol = 2,
#   rel_widths = c(2, 6)   # scatterplot = 2, boxplot+scatterplot+boxplot+scatterplot = 6
# )

final_plot <- plot_grid(
  fig2a_centered, fig2b,
  labels = c("A", "B"),
  rel_widths = c(1, 3),
  ncol = 2
)
...
# plot_grid(
#   plot_grid(fig2a, fig2b, ncol = 1, rel_heights = c(1, 2), labels = "AUTO",   label_size = 24),
#   fig2c,
#   labels = "AUTO",
#   label_size = 24,
#   rel_widths = c(2, 1)
...
# Step 1: combine Whites scatter (fig2a) and your boxplot (fig_box)
fig2a_combo <- plot_grid(
  fig2a, p,
  ncol = 1,
  rel_heights = c(1, 1) # scatterplot taller, boxplot shorter
)

# Step 2: center it the same way you did before
fig2a_centered <- plot_grid(
  NULL, fig2a_combo, NULL,
  ncol = 1,
  rel_heights = c(1, 1, 1)
)

# Step 3: assemble with the rest
final_plot <- plot_grid(
  fig2a_combo, fig2b,
  rel_widths = c(1, 3),
  ncol = 2
...
# saveRDS(final_plot, "plots_RDS/fig2.rds")
...
t_test_results <- function(data, clock, cohort) {
  ad_data <- data %>%
    filter(COHORT == cohort & STATUS == "AD") %>%
    pull(clock)
  control_data <- data %>%
    filter(COHORT == cohort & STATUS == "CONTROL") %>%
    pull(clock)
...
fig3b <- pheatmap(median_diff_matrix,
  ...
  main = "Heatmap of Median Differences in Intrinsic Age Acceleration",
  fontsize_number = 25,
  fontsize_row = 16, # Increase row label font size
  fontsize_col = 16, # Increase column label font size
  fontface_row = "bold", # Make row labels bold
  fontface_col = "bold", # Make column labels bold
  show_rownames = TRUE, # Ensure row names are shown
  treeheight_row = 0, # Remove row dendrogram space
  legend = TRUE,
  annotation_legend = TRUE
)
...
# Filter data to only include CONTROL subjects and create absolute values
controls_only <- bio_age_global_anc %>%
  filter(STATUS == "CONTROL") %>%
  mutate(abs_ageAcc.Horvath = abs(ageAcc.Horvath)) %>%
  filter(!is.na(abs_ageAcc.Horvath)) # Remove NA values

# Check available cohorts
cohorts <- unique(controls_only$COHORT)
cat("Available cohorts with CONTROL subjects:\n")
print(table(controls_only$COHORT))
cat("\n")

# Function to perform pairwise Mann-Whitney U tests between cohorts
perform_pairwise_mann_whitney <- function(data) {
  ...
  for (i in 1:(n_cohorts - 1)) {
    for (j in (i + 1):n_cohorts) {
      cohort1 <- cohorts[i]
      cohort2 <- cohorts[j]

      # Get data for each cohort
      group1 <- data %>%
        filter(COHORT == cohort1) %>%
        pull(abs_ageAcc.Horvath)
      ...
      if (length(group1) > 0 && length(group2) > 0) {
        ...
        # Store results
        results_list[[comparison_count]] <- data.frame(
          comparison = paste(cohort1, "vs", cohort2),
          cohort1 = cohort1,
          cohort2 = cohort2,
          n_cohort1 = length(group1),
          n_cohort2 = length(group2),
          median_cohort1 = median(group1),
          median_cohort2 = median(group2),
        ...
        cat("Comparison:", cohort1, "vs", cohort2, "\n")
        cat(cohort1, "- n:", length(group1), "median:", round(median(group1), 4), "\n")
        cat(cohort2, "- n:", length(group2), "median:", round(median(group2), 4), "\n")
        cat("W statistic:", test_result$statistic, "\n")
        cat("P-value:", round(test_result$p.value, 6), "\n")
        cat("95% CI for difference in location:", round(test_result$conf.int[1], 4), "to", round(test_result$conf.int[2], 4), "\n")
        cat("---\n")
      }
    }
  }
...
# Alternative: If you want to compare each cohort against a reference cohort
perform_reference_comparison <- function(data, reference_cohort) {
  cohorts <- unique(data$COHORT)
  cohorts <- cohorts[cohorts != reference_cohort] # Remove reference from comparison list

  # Get reference group data
  reference_group <- data %>%
    filter(COHORT == reference_cohort) %>%
    pull(abs_ageAcc.Horvath)

  if (length(reference_group) == 0) {
    cat("Reference cohort", reference_cohort, "not found or has no data.\n")
    return(NULL)
  }
  ...
  for (i in 1:length(cohorts)) {
    comparison_cohort <- cohorts[i]

    comparison_group <- data %>%
      filter(COHORT == comparison_cohort) %>%
      pull(abs_ageAcc.Horvath)

    if (length(comparison_group) > 0) {
      test_result <- wilcox.test(reference_group, comparison_group,
        alternative = "two.sided",
        exact = FALSE,
        conf.int = TRUE
      )
      ...
      results_list[[i]] <- data.frame(
        comparison = paste(reference_cohort, "vs", comparison_cohort),
        reference_cohort = reference_cohort,
        comparison_cohort = comparison_cohort,
        n_reference = length(reference_group),
        n_comparison = length(comparison_group),
        median_reference = median(reference_group),
        median_comparison = median(comparison_group),
        W_statistic = test_result$statistic,
        p_value = test_result$p.value,
        conf_int_lower = test_result$conf.int[1],
        conf_int_upper = test_result$conf.int[2],
      ...
      cat("Comparison:", reference_cohort, "vs", comparison_cohort, "\n")
      cat(reference_cohort, "(ref) - n:", length(reference_group), "median:", round(median(reference_group), 4), "\n")
      cat(comparison_cohort, "- n:", length(comparison_group), "median:", round(median(comparison_group), 4), "\n")
      cat("W statistic:", test_result$statistic, "\n")
      cat("P-value:", round(test_result$p.value, 6), "\n")
      cat("95% CI for difference in location:", round(test_result$conf.int[1], 4), "to", round(test_result$conf.int[2], 4), "\n")
      cat("---\n")
...
# Perform all pairwise comparisons between cohorts
cat("=== PAIRWISE COMPARISONS BETWEEN ALL COHORTS ===\n")
pairwise_results <- perform_pairwise_mann_whitney(controls_only)
...
if (!is.null(pairwise_results)) {
  ...
  pairwise_results$p_value_bonferroni <- p.adjust(pairwise_results$p_value, method = "bonferroni")
  pairwise_results$p_value_fdr <- p.adjust(pairwise_results$p_value, method = "BH")

  cat("\n=== RESULTS WITH MULTIPLE TESTING CORRECTIONS ===\n")
  print(pairwise_results)

  # Identify significant comparisons
  cat("\n=== SIGNIFICANT COMPARISONS (p < 0.05) ===\n")
  significant <- pairwise_results[pairwise_results$p_value < 0.05, ]
  if (nrow(significant) > 0) {
    print(significant[, c("comparison", "median_cohort1", "median_cohort2", "p_value", "p_value_fdr")])
  } else {
...
# Uncomment the following lines if you want to compare all cohorts against a specific reference cohort:
# cat("\n=== COMPARISON AGAINST REFERENCE COHORT ===\n")
# reference_cohort <- "COHORT_NAME_HERE"  # Replace with your reference cohort name
# reference_results <- perform_reference_comparison(controls_only, reference_cohort)
